...
for ind in range(0, len(classes)):
    ...
    for line in class_annotations:
        ...
        command = f'aws s3 --no-sign-request --only-show-errors cp s3://open-images-dataset/{run_mode}/{line_parts[0]}.jpg {run_mode}/{class_name}/{line_parts[0]}.jpg'
        commands.append(command)
        
        with open(f'{run_mode}/{class_name}/{line_parts[0]}.txt', 'a') as f:
